...
if (!is.na(subsample)) {
  cat("Subsampling:", subsample, "cells per file\n")
} else {
  cat("Subsampling: disabled (using all cells)\n")
}
...
for (batch in names(batch_dirs)) {
  ...
  for (f in files) {
    ...
    if (!is.na(subsample) && nrow(df) > subsample) {
      df <- df[sample(nrow(df), subsample), ]
    }
